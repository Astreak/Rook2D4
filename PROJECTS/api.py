...
def linkerfashion():
	...
	if request.method=="POST":
		...
		if data.status_code!=200:
			...
			dataf=pd.DataFrame({"PRODUCTS":session["FASHION_PRODUCT"],"RATINGS":session["FASHION_RATINGS"],"PRICES":session["FASHION_PRICE"]})
			oly=dataf.to_csv("data.csv",sep=",")
			path="data.csv"
	...
	if request.method=="POST":
		...
		if data.status_code!=200:
			...
			for d in soup.find_all("div",class_="_3liAhj"):
				...
				if d.find("div",class_="hGSR34")!=None:
						ratings.append(float(d.find("div",class_="hGSR34").get_text()))
				else:
					ratings.append(float(0))
			...
			dataf=pd.DataFrame({"PRODUCTS":session["TECH_PRODUCT"],"RATINGS":session["TECH_RATINGS"],"PRICES":session["TECH_PRICE"]})
			oly=dataf.to_csv("data.csv",sep=",")
			path="data.csv"
			return send_file(path,as_attachment=True)
